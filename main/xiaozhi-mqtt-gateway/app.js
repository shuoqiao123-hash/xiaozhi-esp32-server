...
class MQTTConnection {
    constructor(socket, connectionId, server) {
        ...
        this.lastReportedBatteryLevel = null;
    ...
    handlePublish(publishData) {
        ...
        console.log(`[MQTT PUBLISH] clientId=${this.clientId} topic=${publishData.topic} payload=${publishData.payload}`);
        ...
        this.reportBatteryLevelIfNeeded(json).catch(error => {
            console.error(`上报设备电量失败 clientId=${this.clientId}:`, error.message);
        });
        if (json.type === 'hello') {
            if (json.version !== 3) {
    ...
    async reportBatteryLevelIfNeeded(json) {
        let batteryLevel = null;
        if (json?.type === 'battery' && Number.isInteger(json?.level)) {
            batteryLevel = json.level;
        } else if (Number.isInteger(json?.battery?.level)) {
            batteryLevel = json.battery.level;
        } else if (json?.type === 'mcp' && Number.isInteger(json?.payload?.result?.serverInfo?.battery?.level)) {
            batteryLevel = json.payload.result.serverInfo.battery.level;
        }
        ...
        if (this.lastReportedBatteryLevel === batteryLevel) {
            return;
        }

        const macParts = this.clientId?.split('@@@');
        const rawMac = macParts?.[macParts.length - 1];
        if (!rawMac) {
            return;
        }

        const macAddress = rawMac.replace(/_/g, ':').toLowerCase();
        console.log(`[BATTERY REPORT] clientId=${this.clientId} macAddress=${macAddress} batteryLevel=${batteryLevel}`);
        const managerApiBaseUrl = process.env.MANAGER_API_URL;
        if (!managerApiBaseUrl) {
            console.warn('未配置MANAGER_API_URL，无法上报设备电量');
            return;
        }

        const managerApiSecret = process.env.MANAGER_API_SECRET;
        if (!managerApiSecret) {
            console.warn('未配置MANAGER_API_SECRET，无法上报设备电量');
            return;
        }

        await sendJsonRequest(`${managerApiBaseUrl.replace(/\/$/, '')}/device/battery-level`, 'PUT', {
            'Authorization': `Bearer ${managerApiSecret}`
        }, {
            macAddress,
            batteryLevel
        });
        console.log(`[BATTERY REPORT] upload success macAddress=${macAddress} batteryLevel=${batteryLevel}`);

        this.lastReportedBatteryLevel = batteryLevel;
    }
