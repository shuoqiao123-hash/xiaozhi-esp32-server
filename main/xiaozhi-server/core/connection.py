...
class ConnectionHandler:
    ...
    def chat(self, query, depth=0):
        if query is not None:
            ...
            if depth == 0:
                self.llm_first_token_start_time = time.perf_counter()
                self.llm_first_token_logged_time = None
                self.tts_first_audio_logged = False
        ...
        try:
            for response in llm_responses:
                ...
                if content is not None and len(content) > 0:
                    if depth == 0 and getattr(self, "llm_first_token_start_time", None) is not None:
                        first_token_now = time.perf_counter()
                        self.logger.bind(tag=TAG).info(
                            f"LLM收到请求到首个token耗时: {first_token_now - self.llm_first_token_start_time:.3f}秒"
                        )
                        self.llm_first_token_logged_time = first_token_now
                        self.llm_first_token_start_time = None
